...
class ManageServiceCenter extends SettingsPage
{
    ...
    protected static ?string $title = 'Pengaturan Service Center';
    ...
    {
        $old_files = app(PageSettings::class)->sc_meta_image;
        $new_files = $data['sc_meta_image'] ?? null;
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Section::make('Status')
                    ->description('Aktifkan atau nonaktifkan halaman service center.')
                    ->columnSpanFull()
                    ->schema([
                        ToggleButtons::make('sc_is_active')
                ...
                Section::make('Meta Tags')
                    ->description('Pengaturan meta tags untuk halaman service center.')
                    ->columnSpanFull()
                    ->schema([
                        TextInput::make('sc_meta_title')
                            ->label('Title')
                            ->autocomplete(false)
                            ->columnSpanFull(),
                        TextInput::make('sc_meta_description')
                            ->label('Description')
                            ->autocomplete(false)
                            ->columnSpanFull(),
                        TextInput::make('sc_meta_keywords')
                            ->label('Keywords')
                            ->autocomplete(false)
                            ->columnSpanFull(),
                        FileUpload::make('sc_meta_image')
                            ->label('OG Image')
                            ->image()
                ...
                Section::make('Konten')
                    ->description('Pengaturan konten untuk halaman service center.')
                    ->columnSpanFull()
                    ->schema([
                        TextInput::make('sc_title')
                            ->label('Judul')
                            ->autocomplete(false)
                            ->columnSpanFull()
                            ->required(),
                        Textarea::make('sc_description')
                            ->label('Deskripsi')
                            ->rows(3)
                            ->autocomplete(false)
                            ->columnSpanFull()
