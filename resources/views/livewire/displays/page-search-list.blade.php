<main>
    <section class="container px-8 py-[100px] md:pt-[125px] md:pb-[60px]">
        <div class="flex flex-col gap-[42px]">
            ...
            {{-- Search --}}
            <div x-data="{ search: @entangle('search') }" class="relative w-full md:max-w-3xl md:mx-auto">
    ...
    <section class="bg-[#F4F4F4] md:bg-white">
        <div class="container flex flex-col gap-[42px] px-4 pt-[76px] pb-4 sm:px-6 md:pt-6 md:pb-20">
            {{-- Search Result --}}
            @if(count($result) > 0)
                                    ...
                                    @foreach ($items as $key => $item)
                                        <a href="{{ route('product.detail', [$item->category->slug, $item->slug]) }}" class="flex gap-4">
                                            <div class="rounded-2xl overflow-hidden bg-white">
                                                <img class="aspect-square size-[100px] object-cover" src="{{ $item->getMedia('products')->first()->getUrl() }}" alt="{{ $item->name }}">
                                            </div>
